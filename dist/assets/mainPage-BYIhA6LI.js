import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */import"./components-YsRQu0Q4.js";import{d as M,c as z,g as $,a as D}from"./firebaseConfig-DA3vMik7.js";import{n as j,f as P}from"./trafficService-Bx4fwI7A.js";import{o as N}from"./vendor-DAUOFwH1.js";let c,k=[],m=null,E=5;function H(){const t=document.getElementById("map");if(!t){console.error("Map container #map not found!");return}if(c){console.warn("Map already initialized, skipping...");return}if(typeof L>"u"||!L.map){console.error("Leaflet library not loaded!");return}const e=L.divIcon({className:"user-location-marker",html:'<div class="marker-pulse"></div><div class="marker-dot"></div>',iconSize:[20,20],iconAnchor:[10,10],popupAnchor:[0,-10]}),n=L.divIcon({className:"city-marker",html:'<div class="city-marker-icon">📍</div>',iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-30]});L.divIcon({className:"search-marker",html:'<div class="search-marker-icon">🔍</div>',iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-30]});try{c=L.map("map",{preferCanvas:!1}).setView([49.2827,-123.1207],12),t.style.height=t.style.height||"500px",t.style.width=t.style.width||"100%",setTimeout(()=>{c&&c.invalidateSize()},100),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(c)}catch(i){console.error("Failed to initialize map:",i);return}L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:19,attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors © <a href="https://carto.com/attributions">CARTO</a>'});let o=!1;navigator.geolocation?navigator.geolocation.getCurrentPosition(function(i){const s=i.coords.latitude,l=i.coords.longitude;m={lat:s,lng:l},c.setView([s,l],14),L.marker([s,l],{icon:e}).addTo(c).bindPopup("📍 You are here!").openPopup(),o=!0,I(s,l,E)},function(i){console.warn("Location failed:",i.message),a()}):(console.warn("Geolocation is not supported by this browser."),a());function a(){setTimeout(()=>{o||L.marker([49.2827,-123.1207],{icon:n}).addTo(c).bindPopup("🏔️ Welcome to Vancouver!").openPopup()},1e3)}return c.whenReady(()=>{const i=document.getElementById("map");i&&i.classList.add("map-loaded"),x(),c.on("moveend",()=>{x()}),c.on("zoomend",()=>{x()})}),c}function F(t){if(!c){console.error("Map not initialized");return}if(!t||t.trim()==="")return;const e=`/api/nominatim/search?format=json&q=${encodeURIComponent(t)}&limit=1`;fetch(e,{headers:{Accept:"application/json"}}).then(n=>n.json()).then(n=>{if(n&&n.length>0){const o=n[0],a=parseFloat(o.lat),i=parseFloat(o.lon),s=L.divIcon({className:"search-marker",html:'<div class="search-marker-icon">🔍</div>',iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-30]});L.marker([a,i],{icon:s}).addTo(c).bindPopup(`📍 ${o.display_name}`).openPopup(),c.setView([a,i],14),console.log("Search successful:",o.display_name)}else alert(`Location not found: ${t}`)}).catch(n=>{console.error("Search failed:",n),alert("Search failed, please try again later")})}function J(){const t=document.getElementById("reportBtn");t&&t.addEventListener("click",function(){window.location.href="./report.html"})}document.addEventListener("DOMContentLoaded",function(){if(!document.getElementById("map")){console.error("Map container not found!");return}function e(){if(typeof L<"u"&&L.map)try{H()}catch(o){console.error("Error initializing map:",o),setTimeout(e,200)}else setTimeout(e,100)}e(),setTimeout(()=>{J()},500),setTimeout(()=>{G()},500),setTimeout(()=>{const o=document.querySelector("app-search-bar");o&&o.addEventListener("search",a=>{const i=a.detail.query;F(i)})},500),!m&&navigator.geolocation&&navigator.geolocation.getCurrentPosition(o=>{m={lat:o.coords.latitude,lng:o.coords.longitude},I(m.lat,m.lng,E)},o=>{console.warn("Failed to get location for traffic jam alert:",o),C("Unable to get your location")});const n=document.getElementById("distanceFilter");n&&n.addEventListener("change",o=>{const a=parseInt(o.target.value);E=a;const i=document.getElementById("trafficJamSubtitle");i&&(i.textContent=`Within ${a}km radius`),m&&I(m.lat,m.lng,a)})});async function x(){if(!c||!M){console.log("Map or Firestore not available, skipping traffic reports");return}try{const t=c.getBounds(),e={minLat:t.getSouth(),maxLat:t.getNorth(),minLng:t.getWest(),maxLng:t.getEast()},n=await P(M,e,2);O(),V(n),console.log(`Loaded ${n.length} traffic reports for current view`)}catch(t){console.error("Failed to load traffic reports:",t)}}function O(){c&&(k.forEach(t=>{c.hasLayer(t)&&c.removeLayer(t)}),k=[])}function V(t){!t||!Array.isArray(t)||t.length===0||t.forEach(e=>{if(typeof e.lat!="number"||typeof e.lng!="number"){console.warn("Invalid report position:",e);return}const n=[e.lat,e.lng],o={username:e.username||"Anonymous",type:e.type||"unknown",comment:e.comment||"",createdAt:e.createdAt||e.timestamp};q(n,{tooltip:"点击查看交通状况",report:o})})}function q(t,e={}){if(!c)return console.error("Map not initialized"),null;if(!t||t.length<2)return console.error("Invalid position:",t),null;const n={tooltip:"查看交通状况",onClick:null,iconSize:[40,40],...e},o=W(n);try{const a=L.marker([t[0],t[1]],{icon:o,zIndexOffset:1e3,riseOnHover:!0}).addTo(c);return U(a,t,n),k.push(a),a}catch(a){return console.error("Error adding traffic marker:",a),null}}function W(t){const e=`
    <div style="
